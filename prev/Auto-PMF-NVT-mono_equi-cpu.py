...
import numpy
import math
from cal_ave import freeMD
from smd_forward import smd_forward_MD
from smd_reverse import smd_reverse_MD
from us_NVT import us_mkdir 
from us_NVT import us_restraint_setup
from us_NVT_2 import us2_NVT
from wham import write_wham
#from force_optimize import sample_force
#from force_optimize import collect_result
from check_force import check_force
from check_force import check_force2
from check_equi import check_equi
from plot import plot
from check_metafile import check_wham
#from rerun_before5A import rerun_before5A
#from rerun_after5A import rerun_after5A

#----------------------------------------------------------------------------#
#                            Main Program                                    #
#----------------------------------------------------------------------------#
parser = OptionParser("Usage: Auto-PMF.py -i inputfile -s/--step step_number \n")
...
#if options.step not in ['prepare', 'sampling','check_force','check_equi','write_wham','check_wham','result','adaptive_force']:
#    raise pymsmtError('Invalid step number chosen. please choose among the following values: prepare, sampling,check_force,check_equi,write_wham,check_wham,result,adaptive_force')
...
for line in rinput:
    ...
    elif line[0].lower() == "smd_forward_min_steps":
        smd_forward_min_steps = int(line[1])
    elif line[0].lower() == "smd_forward_nvt_steps":
        smd_forward_nvt_steps = int(line[1])
    elif line[0].lower() == "smd_forward_npt_steps":
        smd_forward_npt_steps = int(line[1])
    elif line[0].lower() == "smd_forward_md_steps":
        smd_forward_md_steps = int(line[1])
    elif line[0].lower() == "forward_limit":
        forward_limit = line[1]
    elif line[0].lower() == "smd_reverse_min_steps":
        smd_reverse_min_steps = int(line[1])
    elif line[0].lower() == "smd_reverse_nvt_steps":
        smd_reverse_nvt_steps = int(line[1])
    elif line[0].lower() == "smd_reverse_npt_steps":
        smd_reverse_npt_steps = int(line[1])
    elif line[0].lower() == "smd_reverse_md_steps":
        smd_reverse_md_steps = int(line[1])
    elif line[0].lower() == "reverse_limit":
        reverse_limit = line[1]
    elif line[0].lower() == "us_min_steps":
        us_min_steps = int(line[1])
    elif line[0].lower() == "us_nvt_steps":
        us_nvt_steps = int(line[1])
...
if mode == "test":
    ...
    md_npt_steps = 500
    md_md_steps = 2000
    smd_forward_min_steps = 500
    smd_forward_nvt_steps = 20000
    smd_forward_npt_steps = 500
    smd_forward_md_steps = 2000
    smd_reverse_min_steps = 500
    smd_reverse_nvt_steps = 20000
    smd_reverse_npt_steps = 500
...
#except:
#    raise auotPMF_error('prmtop needs to be provided.')

#try:
#    print('The variable inpcrd is : ', inpcrd)
#except:
#    raise auotPMF_error('inpcrd needs to be provided.')

#try:
#    print('The variable forward_limit is : ', forward_limit)
#except:
#    raise auotPMF_error('forward_limit needs to be provided.')
#
#try:
...
if (options.step == 'prepare'):
  ...
  print('The variable smd_forward_min_steps is : ', smd_forward_min_steps)
  print('The variable smd_forward_nvt_steps is : ', smd_forward_nvt_steps)
  print('The variable smd_forward_npt_steps is : ', smd_forward_npt_steps)
  print('The variable smd_forward_md_steps is : ', smd_forward_md_steps)
  print('The variable forward_limit is : ', forward_limit)
  print('The variable smd_reverse_min_steps is : ', smd_reverse_min_steps)
  print('The variable smd_reverse_nvt_steps is : ', smd_reverse_nvt_steps)
  print('The variable smd_reverse_npt_steps is : ', smd_reverse_npt_steps)
  print('The variable smd_reverse_md_steps is : ', smd_reverse_md_steps)
  print('The variable reverse_limit is : ', reverse_limit)
  print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

  md_equi_RST=open("md_dis.RST","w")
  print("# restraint",file=md_equi_RST)
  print(" &rst  iat=-1, -1, r1=0.0, r2=%s, r3=%s, r4=100., rk2=50, rk3=50,igr1=%s,igr2=%s,/"%(equi_dis,equi_dis,coord_atm1_num,coord_atm2_num),file=md_equi_RST)
  md_equi_RST.close()


  avg_res_val,fram_num = freeMD(prmtop, inpcrd, md_min_steps, md_nvt_steps, md_npt_steps, md_md_steps,coord_atm1_num,coord_atm2_num)
  avg_res_val=round(float(avg_res_val),2)
  info=open("freemd.txt","w")
  print ("%s %s"%(avg_res_val,fram_num),file=info)
  info.close()

  smd_forward_MD(prmtop, inpcrd, smd_forward_min_steps, smd_forward_nvt_steps, smd_forward_npt_steps, smd_forward_md_steps, coord_atm1_num, coord_atm2_num,forward_limit,avg_res_val,fram_num)
  smd_reverse_MD(prmtop, inpcrd, smd_reverse_min_steps, smd_reverse_nvt_steps, smd_reverse_npt_steps, smd_reverse_md_steps, coord_atm1_num, coord_atm2_num,reverse_limit,avg_res_val,fram_num)

if (options.step == 'sampling'):
  ...
  if line_num > 1:
    ...
    for space_number in range(1,space_num+1):
       us_mkdir(prmtop, inpcrd, avg_res_val, us_min_steps, us_nvt_steps, us_npt_steps, us_md_steps,us_window_space,space_number,slm_header)

    for restraint_number in range(1,restraint_num+1):
     us_restraint_setup(prmtop, inpcrd, coord_atm1_num, coord_atm2_num, us_window_restraint, restraint_number, forward_limit)
  ...
  for window in dir_list:
     ...
     for optimize_number in range(1,optimize_num+1):
       ...
       if window >= ini and window < fin:
         os.system("cp ~/auto-pmf/adaptive.py .")
  ...
  for window in dir_list:
     ...
     for optimize_number in range(1,optimize_num+1):
       ...
       center_deviation = float(center_deviation)
       if window >= ini and window < fin:
         os.system("cp ~/auto-pmf/adaptive2.py .")
...
elif (options.step == 'check_equi'):
  for optimize_number in range(1,optimize_num+1):
       check_equi(us_window_optimize, optimize_number, us_window_restraint, center_deviation)
  ...
  for space_number in range(1,space_num+1):
    us2_NVT(prmtop, inpcrd, avg_res_val, us_min_steps, us_nvt_steps, us_npt_steps, us_md_steps,us_window_space,space_number,slm_header)
