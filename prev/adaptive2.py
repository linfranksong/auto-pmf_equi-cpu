...
if lower_b < round(float(WINDOW),2) and round(float(WINDOW),2) < upper_b:
   if "adapt2.RST" not in os.listdir('.'):
     ...
     if us_window_restraint != rst2:
       ...
       slm = open("command",'w')
       slm.write("pmemd.cuda -O -i us_min.in -o us_min.out -p PRMTOP -c start.rst -r us_min.rst -x us_min.netcdf -ref start.rst\n")
       slm.write("pmemd.cuda -O -i us_nvt.in -o us_nvt.out -p PRMTOP -c us_min.rst -r us_nvt.rst -x us_nvt.netcdf -ref us_min.rst\n")
       slm.write("mpirun -np 4 pmemd.MPI -O -i us_snpt.in -o us_snpt.out -p PRMTOP -c us_nvt.rst -r us_snpt.rst -x us_snpt.netcdf -ref us_nvt.rst\n")
       slm.write("pmemd.cuda -O -i us_npt.in -o us_npt.out -p PRMTOP -c us_snpt.rst -r us_npt.rst -x us_npt.netcdf -ref us_snpt.rst\n")
