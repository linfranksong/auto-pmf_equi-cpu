...
def print_inputf(file_name, nxt, window_steps):
    ...
    if nxt in ['min1','min2','min3','min4','min5','npt','snpt']:
        ...
        print("DUMPAVE=%s_smd_forward.log"%(nxt),file=md_mdf)
    elif nxt == 'nvt':
...
def smd_forward_MD(prmtop, inpcrd, smd_forward_min_steps, smd_forward_nvt_steps, smd_forward_npt_steps, smd_forward_md_steps, coord_atm1_num, coord_atm2_num,forward_limit,aveg_res_val,fram_num):
    ...
    smd_forward_md_steps = int((float(forward_limit)-float(aveg_res_val))*1000000)

    #smd MD simulation
    print("****Perform smd forward MD simulation ...")

    #MIN
    print("Perform smd_forward_min1 %d steps..." %smd_forward_min_steps)
    print_inputf('smd_forward_min1.in', 'min1', smd_forward_min_steps)
    os.system("pmemd.cuda -O -i smd_forward_min1.in -o smd_forward_min1.out -p %s -c md_md_center_%s.rst -r smd_forward_min1.rst -x smd_forward_min1.netcdf -ref md_md_center_%s.rst" %(prmtop,fram_num,fram_num))

    print("Perform smd_forward_min2 %d steps..." %smd_forward_min_steps)
    print_inputf('smd_forward_min2.in', 'min2', smd_forward_min_steps)
    os.system("pmemd.cuda -O -i smd_forward_min2.in -o smd_forward_min2.out -p %s -c smd_forward_min1.rst -r smd_forward_min2.rst -x smd_forward_min2.netcdf -ref smd_forward_min1.rst" %(prmtop))

    print("Perform smd_forward_min3 %d steps..." %smd_forward_min_steps)
    print_inputf('smd_forward_min3.in', 'min3', smd_forward_min_steps)
    os.system("pmemd.cuda -O -i smd_forward_min3.in -o smd_forward_min3.out -p %s -c smd_forward_min2.rst -r smd_forward_min3.rst -x smd_forward_min3.netcdf -ref smd_forward_min2.rst" %(prmtop))

    print("Perform smd_forward_min4 %d steps..." %smd_forward_min_steps)
    print_inputf('smd_forward_min4.in', 'min4', smd_forward_min_steps)
    os.system("pmemd.cuda -O -i smd_forward_min4.in -o smd_forward_min4.out -p %s -c smd_forward_min3.rst -r smd_forward_min4.rst -x smd_forward_min4.netcdf -ref smd_forward_min3.rst" %(prmtop))

    print("Perform smd_forward_min5 %d steps..." %smd_forward_min_steps)
    print_inputf('smd_forward_min5.in', 'min5', smd_forward_min_steps)
    os.system("pmemd.cuda -O -i smd_forward_min5.in -o smd_forward_min5.out -p %s -c smd_forward_min4.rst -r smd_forward_min5.rst -x smd_forward_min5.netcdf -ref smd_forward_min4.rst" %(prmtop))

    #NVT
    print("Perform smd_forward_nvt %d steps..." %smd_forward_nvt_steps)
    print_inputf('smd_forward_nvt.in', 'nvt', smd_forward_nvt_steps)
    os.system("pmemd.cuda -O -i smd_forward_nvt.in -o smd_forward_nvt.out -p %s -c smd_forward_min5.rst -r smd_forward_nvt.rst -x smd_forward_nvt.netcdf -ref smd_forward_min5.rst" %(prmtop))

    #SNPT
    print("Perform smd_forward_snpt 50000 ...")
    print_inputf('smd_forward_snpt.in', 'snpt', 50000)
    os.system("mpirun -np 4 pmemd.MPI -O -i smd_forward_snpt.in -o smd_forward_snpt.out -p %s -c smd_forward_nvt.rst -r smd_forward_snpt.rst -x smd_forward_snpt.netcdf -ref smd_forward_nvt.rst" %(prmtop))

    #NPT
    print("Perform smd_forward_npt %d steps..." %smd_forward_npt_steps)
    print_inputf('smd_forward_npt.in', 'npt', smd_forward_npt_steps)
    os.system("pmemd.cuda -O -i smd_forward_npt.in -o smd_forward_npt.out -p %s -c smd_forward_snpt.rst -r smd_forward_npt.rst -x smd_forward_npt.netcdf -ref smd_forward_snpt.rst" %(prmtop))

    #MD
    print("Perform smd_forward_md %d steps..." %smd_forward_md_steps)
    print_inputf('smd_forward_md.in', 'md', smd_forward_md_steps)
    os.system("pmemd.cuda -O -i smd_forward_md.in -o smd_forward_md.out -p %s -c smd_forward_npt.rst -r smd_forward_md.rst -x smd_forward_md.netcdf -ref smd_forward_npt.rst" %(prmtop))
